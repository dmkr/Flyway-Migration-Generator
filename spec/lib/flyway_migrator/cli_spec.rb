...
RSpec.describe FlywayMigrator::CLI do
  let(:options) { [] }
  ...
  shared_examples_for "a version command" do
    it "prints version" do
      result = -> { cli }
      expect(&result).to output(/#{FlywayMigrator::Identity.version_label}\n/).to_stdout
    end
  end
  ...
  shared_examples_for "a help command" do
    it "prints usage" do
      result = -> { cli }
      expect(&result).to output(/#{FlywayMigrator::Identity.version_label}\scommands:\n/).to_stdout
    end
  end
